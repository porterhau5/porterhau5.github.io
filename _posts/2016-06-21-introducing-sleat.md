...
the Inspired by Victor Mata's method of searching Windows logon events for identifying locations of privileged users. The attack vector seemed like an opportunity to automate this foraging process a bit, especially when it comes to scope validation or when the name of a privileged user is unknown. This framework could be extended to process other types of Windows Events.
